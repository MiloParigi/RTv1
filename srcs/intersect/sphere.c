...
float	intersect_sphere(t_ray ray, t_obj *sphere)
{
	...
	{
		plimit_dist = intersect_obj(ray, *(sphere->plimit));
		t_vec3 point_of_impact = vec_add3(ray.pos, vec_scale3(ray.dir, plimit_dist));
		float result = vec_dot3(ray.dir, object_norm(*(sphere->plimit), point_of_impact)); // vec_dot3(ray.dir, sphere->plimit->vector);
		if (result > 0)
		{
