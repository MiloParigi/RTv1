...
/*   Updated: 2017/10/02 18:37:53 by agfernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color			switch_color_func(t_rt *e, t_ray ray, t_reflect ref, float min_dist)
{
	t_color		temp_color;
...
static t_color	get_pxl_color(t_rt *e, t_ray ray)
{
	...
	if (e->scene.id == -1)
		return (ref.color);
	return (switch_color_func(e, ray, ref, min_dist));
}
